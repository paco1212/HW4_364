...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/frgaHW4db" 
...
# Set up association Table between search terms and GIFs (you can call it anything you want, we suggest 'tags' or 'search_gifs').
search_gif_collections = db.Table('search_gifs', db.Column('search_id',db.Integer, db.ForeignKey('searches.id')), db.Column('gif_id', db.Integer, db.ForeignKey('gifs.id')))



# Set up association Table between GIFs and collections prepared by user (you can call it anything you want. We suggest: user_collection)
user_collection = db.Table('user_collections', db.Column('gif_id', db.Integer, db.ForeignKey('gifs.id')), db.Column('collection_id', db.Integer, db.ForeignKey('gif_collections.id')))
...
class User(UserMixin, db.Model):
    ...
    # setting up one to many relationship with PersonalGifCollection table/model
    gif_collection = db.relationship('PersonalGifCollection', backref = 'personal_gifs')
...
class Gif(db.Model):
    ...
    # Define a __repr__ method for the Gif model that shows the title and the URL of the gif
    def __repr__(self):
        return '%s | %s' % (self.title, self.embedURL)

# Model to store a personal gif collection
class PersonalGifCollection(db.Model):
    __tablename__ = 'gif_collections'
    id = db.Column(db.Integer, primary_key = True) # id (Integer, primary key)
    name = db.Column(db.String(255)) # name (String, up to 255 characters)
    user_id = db.Column(db.Integer, db.ForeignKey("users.id")) # This model should have a one-to-many relationship with the User model (one user, many personal collections of gifs with different names -- say, "Happy Gif Collection" or "Sad Gif Collection")
    ...
    gifs = db.relationship('Gif', secondary = user_collection, backref = db.backref('gif_collections', lazy = 'dynamic'), lazy = 'dynamic')
...
class SearchTerm(db.Model):
    ...
    gifs = db.relationship('Gif', secondary = search_gif_collections, backref=db.backref('searches', lazy='dynamic'),lazy='dynamic')

    # Define a __repr__ method for this model class that returns the term string
    def __repr__(self):
        return "%s" % (self.term)
...
# The following forms for searching for gifs and creating collections are provided and should not be edited. You SHOULD examine them so you understand what data they pass along and can investigate as you build your view functions in TODOs below.
...
# This function should make a request to the Giphy API using the input search_string, and your api_key (imported at the top of this file)
...
# This function should get or create a Gif instance. Determining whether the gif already exists in the database should be based on the gif's title and URL.
...
def get_or_create_search_term(term):
    ...
    # This function should return the search term instance if it already exists.
    ...
    # This function should invoke the get_gifs_from_giphy function to get a list of gif data from Giphy.
        json_obj = get_gifs_from_giphy(term)
    ...
    # It should iterate over that list acquired from Giphy and invoke get_or_create_gif for each, and then append the return value from get_or_create_gif to the search term's associated gifs (remember, many-to-many relationship between search terms and gifs, allowing you to do this!).
        gifs = [get_or_create_gif(diction['title'], diction['embed_url']) for diction in json_obj]
...
# This function should get or create a personal gif collection. Uniqueness of the gif collection should be determined by the name of the collection and the id of the logged in user.
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    # However, if no such collection exists, a new PersonalGifCollection instance should be created, and each Gif in the gif_list input should be appended to it (remember, there exists a many to many relationship between Gifs and PersonalGifCollection).
    if not collection:
        collection = PersonalGifCollection(name = name, user_id = current_user.id)
        collection.gifs.extend(gif_list)
...
def index():
    # This view function renders the GifSearchForm.
    form = GifSearchForm()
    ...
    # invoke get_or_create_search_term on the form input and redirect to the function corresponding to the path /gifs_searched/<search_term> in order to see the results of the gif search. 
        ...
        search_term_obj = get_or_create_search_term(search_query)
        return redirect(url_for('search_results',search_term = search_query))

    # invoking url_for with a named argument will send additional data. e.g. url_for('artist_info',artist='solange') would send the data 'solange' to a route /artist_info/<artist>
...
def create_collection():
    ...
    # If the form validates on submit, get the list of the gif ids
...
@login_required
def collections():
    # This view function should render the collections.html template so that only the current user's personal gif collection links will render in that template. Make sure to examine the template so that you send it the correct data!
    user = current_user
    user_collection = User.query.filter_by(id = user.id).first().gif_collection
